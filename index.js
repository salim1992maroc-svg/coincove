...
function renderApp(){return String.raw`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>Coin Cove</title><script src="https://telegram.org/js/telegram-web-app.js"></script><script src="https://libtl.com/sdk.js" data-zone="11766606" data-sdk="show_11766606"></script><style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#f5f7fb;color:#111827}.app{max-width:560px;margin:auto;padding:22px 16px 90px}.card{background:#fff;border-radius:20px;padding:18px;margin:12px 0;box-shadow:0 4px 18px #0000000b}.balance{background:#111827;color:#fff;border-radius:24px;padding:24px}.num{font-size:38px;font-weight:800}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.btn{width:100%;border:0;border-radius:14px;padding:13px;font-weight:700;background:#111827;color:#fff}.input{width:100%;padding:13px;border:1px solid #ddd;border-radius:12px;margin:7px 0}.muted{opacity:.6;font-size:13px}.hidden{display:none}.nav{position:fixed;bottom:0;left:0;right:0;background:#fff;padding:10px;display:flex;justify-content:center;gap:8px}.nav button{border:0;background:none;padding:8px}.modal{position:fixed;inset:0;background:#0008;display:flex;align-items:flex-end;padding:12px}.sheet{background:#fff;border-radius:22px;padding:20px;width:100%;max-width:560px;margin:auto}</style></head><body><div id="app" class="app">Loading Coin Cove...</div><script>(function(){const tg=window.Telegram&&window.Telegram.WebApp;let user=null;const $=id=>document.getElementById(id);if(tg){try{tg.ready();tg.expand()}catch(e){}}function init(){return tg&&tg.initData?tg.initData:""}async function req(path,opt={}){const et=localStorage.getItem('cc_email_token')||''; opt.headers=Object.assign({},opt.headers||{},init()?{"X-Telegram-Init-Data":init()}:(!opt.headers?.Authorization&&et?{"Authorization":"Bearer "+et}:{}));const r=await fetch(path,opt);return r.json()}async function load(){try{const telegramInit=init();const emailToken=localStorage.getItem('cc_email_token')||'';let d;if(telegramInit){d=await req('/api/me')}else if(emailToken){d=await req('/api/auth/me')}else{showAuth();return}if(!d.success){if(!telegramInit&&emailToken)localStorage.removeItem('cc_email_token');showAuth();return}user=d.user;render(d)}catch(e){console.error('load:',e);showError('Unable to connect.')}}function render(d){const w=d.wallet||{};const u=d.user||{};$('app').innerHTML='<div><h2>🪙 Coin Cove</h2><div class="balance"><div>Welcome, '+esc(u.first_name||'there')+'</div><div class="num">'+Number(w.balance||0).toLocaleString()+'</div><div>Coins</div></div><div class="grid"><button class="btn" onclick="offers()">🎁 Offers</button><button class="btn" onclick="ad()">📺 Watch & Earn</button><button class="btn" onclick="ref()">👥 Invite</button><button class="btn" onclick="withdraw()">💸 Withdraw</button></div><div class="card"><b>Recent Activity</b><div class="muted">'+(d.transactions||[]).map(x=>esc(x.description||x.type)+' : '+esc(x.amount)).join('<br>')+'</div></div></div><div class="nav"><button onclick="load()">🏠 Home</button><button onclick="offers()">🎁 Offers</button><button onclick="ref()">👥 Invite</button><button onclick="withdraw()">💰 Wallet</button></div>'}function showAuth(){$('app').innerHTML='<div class="card"><h2>🪙 Coin Cove</h2><p>Sign in with your email to use Coin Cove independently from Telegram.</p><button class="btn" onclick="email()">Email Login / Register</button></div>'}function showError(m){$('app').innerHTML='<div class="card"><h2>Coin Cove</h2><p>'+esc(m||'Error')+'</p><button class="btn" onclick="load()">Retry</button></div>'}window.offers=()=>{if(!user)return;const uid=encodeURIComponent(String(user.telegram_id||user.id));document.body.insertAdjacentHTML('beforeend','<div id="wall" class="modal"><div class="sheet"><button class="btn" onclick="closeWall()">Close</button><iframe style="width:100%;height:75vh;border:0" src="https://offerwall.gg/wall/4c826098db679c99583194371d0eae1b?userId='+uid+'"></iframe></div></div>')};window.closeWall=()=>document.getElementById('wall')?.remove();window.ad=async()=>{if(!tg||!tg.initData)return alert('Open from Telegram.');if(typeof window.show_11766606!=='function')return alert('Ad service is not ready.');try{await window.show_11766606({ymid:String(user.telegram_id),requestVar:'watch_earn'});const d=await req('/api/reward-ad',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({initData:tg.initData})});alert(d.message||'Completed');setTimeout(load,2500)}catch(e){alert('Ad was not completed.')}};window.ref=async()=>{const d=await req('/api/referral');if(!d.success)return alert(d.message);prompt('Referral link',d.link)};window.withdraw=()=>{document.body.insertAdjacentHTML('beforeend','<div id="wd" class="modal"><div class="sheet"><h3>Withdraw</h3><p class="muted">Minimum 200 Coins = $0.10</p><select id="wm" class="input"><option value="USDT_TRC20">USDT TRC20</option><option value="BINANCE_ID">BINANCE ID</option></select><input id="wc" class="input" type="number" min="200" step="200" placeholder="Coins"><input id="dest" class="input" placeholder="Wallet address / Binance ID"><button class="btn" onclick="submitWd()">Submit</button><br><br><button class="btn" onclick="document.getElementById(\'wd\').remove()">Cancel</button></div></div>')};window.submitWd=async()=>{const b={method:document.getElementById('wm').value,coins:Number(document.getElementById('wc').value),destination:document.getElementById('dest').value};const d=await req('/api/withdraw',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});alert(d.message|| (d.success?'Withdrawal submitted':'Failed'));if(d.success){document.getElementById('wd').remove();load()}};window.email=()=>{if(document.getElementById('em'))return;document.body.insertAdjacentHTML('beforeend','<div id="em" class="modal"><div class="sheet"><h3>Email account</h3><p class="muted">Create an account without Telegram. No email verification is required.</p><input id="ee" class="input" type="email" autocomplete="email" placeholder="Email"><input id="ep" class="input" type="password" autocomplete="new-password" placeholder="Password (minimum 8 characters)"><input id="ec" class="input" type="password" autocomplete="new-password" placeholder="Confirm Password"><div id="emsg" class="muted" style="min-height:20px;margin:8px 0;color:#b91c1c"></div><button id="regBtn" class="btn" onclick="reg()">Create Account</button><br><br><button id="loginBtn" class="btn" onclick="login()">Login</button><br><br><button class="btn" onclick="document.getElementById(\'em\').remove()">Close</button></div></div>')};async function emailRequest(path,body){const r=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(body),cache:'no-store'});let d;try{d=await r.json()}catch(e){d={success:false,message:'Server returned an invalid response ('+r.status+').'}};return {r:r,d:d}}window.reg=async()=>{const e=document.getElementById('ee'),p=document.getElementById('ep'),c=document.getElementById('ec'),msg=document.getElementById('emsg'),btn=document.getElementById('regBtn');const email=String(e.value||'').trim().toLowerCase(),password=String(p.value||''),confirmPassword=String(c.value||'');msg.textContent='';if(!email||!email.includes('@')){msg.textContent='Enter a valid email.';return}if(password.length<8){msg.textContent='Password must be at least 8 characters.';return}if(password!==confirmPassword){msg.textContent='Passwords do not match.';return}btn.disabled=true;btn.textContent='Creating...';try{const x=await emailRequest('/api/auth/register',{email,password,confirmPassword});const d=x.d;if(d.success&&d.token){localStorage.setItem('cc_email_token',d.token);msg.style.color='#15803d';msg.textContent='Account created successfully. Loading...';setTimeout(()=>location.reload(),300);return}msg.textContent=d.message||('Registration failed ('+x.r.status+').')}catch(err){console.error('Email registration:',err);msg.textContent='Network error. Please try again.'}finally{btn.disabled=false;btn.textContent='Create Account'}};window.login=async()=>{const e=document.getElementById('ee'),p=document.getElementById('ep'),msg=document.getElementById('emsg'),btn=document.getElementById('loginBtn');const email=String(e.value||'').trim().toLowerCase(),password=String(p.value||'');msg.style.color='#b91c1c';msg.textContent='';if(!email||!password){msg.textContent='Enter your email and password.';return}btn.disabled=true;btn.textContent='Logging in...';try{const x=await emailRequest('/api/auth/login',{email,password});const d=x.d;if(d.success&&d.token){localStorage.setItem('cc_email_token',d.token);msg.style.color='#15803d';msg.textContent='Login successful. Loading...';setTimeout(()=>location.reload(),300);return}msg.textContent=d.message||('Login failed ('+x.r.status+').')}catch(err){console.error('Email login:',err);msg.textContent='Network error. Please try again.'}finally{btn.disabled=false;btn.textContent='Login'}};function esc(v){return String(v??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#039;')}load()})();</script></body></html>`}
